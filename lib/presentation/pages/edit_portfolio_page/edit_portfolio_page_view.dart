...
import 'package:norm/presentation/global/portfolio_wrapper/portfolio_wrapper.dart';
...
class EditPortfolioPageView extends StatelessWidget {
  const EditPortfolioPageView({super.key});
  ...
  Widget _buildSpecialityPicker(BuildContext context) {
    return Observer(
      builder: (context) {
        final EditPortfolioState state = context.watch<EditPortfolioState>();
  ...
  Widget _buildAddPortfolioButton(BuildContext context) {
    return GestureDetector(
      onTap: () async {
        final EditPortfolioState state = context.read<EditPortfolioState>();

        final Portfolio? portfolio = await service<AppRouter>().goToCreatePortfolioPage();
  ...
  Widget _buildPortfolioItems(BuildContext context) {
    return Observer(
      builder: (context) {
        final EditPortfolioState state = context.watch<EditPortfolioState>();
        ...
        return TwoItemsSquareGrid(
          itemBuilder: (context, index) {
            if (index == 0) {
              return _buildAddPortfolioButton(context);
            }

            final Portfolio portfolio = state.portfolio.elementAt(index - 1);

            return PortfolioWrapper(portfolio: portfolio);
          },
          itemCount: state.portfolio.length + 1,
        );
      },
    );
  }
