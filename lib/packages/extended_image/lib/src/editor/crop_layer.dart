...
enum MoveType { topLeft, topRight, bottomRight, bottomLeft, top, right, bottom, left }
...
class ExtendedImageCropLayerState extends State<ExtendedImageCropLayer> with SingleTickerProviderStateMixin {
  ...
  set cropRect(Rect? value) {
    ...
    widget.editorConfig.editActionDetailsIsChanged?.call(widget.editActionDetails);
  }

  bool get isAnimating => _rectTweenController.isAnimating;
  bool get isMoving => currentMoveType != null;
  ...
  void initState() {
    super.initState();
    _pointerDown = false;
    _rectTweenController = AnimationController(vsync: this, duration: widget.editorConfig.animationDuration)
      ..addListener(doCropAutoCenterAnimation);
  ...
  Widget build(BuildContext context) {
    ...
    final Color maskColor = widget.editorConfig.editorMaskColorHandler?.call(context, _pointerDown) ??
    ...
    final Widget result = CustomPaint(
      painter: ExtendedImageCropLayerPainter(
        ...
        lineColor: editConfig.lineColor ?? Theme.of(context).scaffoldBackgroundColor.withOpacity(0.7),
  ...
  void moveUpdate(MoveType moveType, Offset delta) {
    ...
    if (currentMoveType != null && moveType != currentMoveType) {
      return;
    }
    currentMoveType = moveType;

    final Rect? layerDestinationRect = widget.editActionDetails.layerDestinationRect;
    ...
    switch (moveType) {
      ...
      case MoveType.left:
        ...
        topLeft = Offset(min(topLeft.dx, result.right - gWidth * 2), min(topLeft.dy, result.bottom - gWidth * 2));
        result = Rect.fromPoints(topLeft, result.bottomRight);
        break;
      case MoveType.topRight:
        Offset topRight = result!.topRight + delta;
        topRight = Offset(max(topRight.dx, result.left + gWidth * 2), min(topRight.dy, result.bottom - gWidth * 2));
      ...
      case MoveType.bottom:
        ...
        bottomRight =
            Offset(max(bottomRight.dx, result.left + gWidth * 2), max(bottomRight.dy, result.top + gWidth * 2));
        result = Rect.fromPoints(result.topLeft, bottomRight);
        break;
      case MoveType.bottomLeft:
        Offset bottomLeft = result!.bottomLeft + delta;
        bottomLeft = Offset(min(bottomLeft.dx, result.right - gWidth * 2), max(bottomLeft.dy, result.top + gWidth * 2));
    ...
    result = Rect.fromPoints(
        Offset(max(result!.left, layerDestinationRect!.left), max(result.top, layerDestinationRect.top)),
        Offset(min(result.right, layerDestinationRect.right), min(result.bottom, layerDestinationRect.bottom)));
    ...
    if (result.beyond(layoutRect)) {
      cropRect = result;
      final Rect centerCropRect = getDestinationRect(rect: layoutRect, inputSize: result.size, fit: widget.fit);
      final Rect newScreenCropRect = centerCropRect.shift(widget.editActionDetails.layoutTopLeft!);
      doCropAutoCenterAnimation(newScreenCropRect: newScreenCropRect);
    } else {
  ...
  Rect _handleAspectRatio(double gWidth, MoveType moveType, Rect result, Rect? layerDestinationRect, Offset delta) {
    ...
    if (aspectRatio != null) {
      ...
      switch (moveType) {
        ...
        case MoveType.bottom:
          ...
          result = _doAspectRatioV(minD, result, aspectRatio, layerDestinationRect!, isTop: isTop);
          break;
        case MoveType.left:
        case MoveType.right:
          final bool isLeft = moveType == MoveType.left;
          result = _doAspectRatioH(minD, result, aspectRatio, layerDestinationRect!, isLeft: isLeft);
          break;
        case MoveType.topLeft:
        case MoveType.topRight:
        case MoveType.bottomRight:
  ...
  ///horizontal
  Rect _doAspectRatioH(double minD, Rect result, double aspectRatio, Rect layerDestinationRect,
  ...
  ///vertical
  Rect _doAspectRatioV(double minD, Rect result, double aspectRatio, Rect layerDestinationRect, {required bool isTop}) {
  ...
  Rect? _doWithMaxScale(Rect rect) {
    final Rect centerCropRect = getDestinationRect(rect: layoutRect, inputSize: rect.size, fit: widget.fit);
    final Rect newScreenCropRect = centerCropRect.shift(widget.editActionDetails.layoutTopLeft!);
  ...
  void moveEnd(MoveType moveType) {
    if (currentMoveType != null && moveType == currentMoveType) {
  ...
  void startTimer() {
    ...
    _timer = Timer.periodic(widget.editorConfig.tickerDuration, (Timer timer) {
      ...
      final Rect centerCropRect = getDestinationRect(rect: layoutRect, inputSize: cropRect!.size, fit: widget.fit);
      final Rect newScreenCropRect = centerCropRect.shift(widget.editActionDetails.layoutTopLeft!);
  ...
  void doCropAutoCenterAnimation({Rect? newScreenCropRect}) {
    if (mounted) {
      setState(() {
        final Rect oldScreenCropRect = widget.editActionDetails.screenCropRect!;
        ...
        final Offset offset = newScreenCropRect!.center - oldScreenCropRect.center;

        /// scale then move
        /// so we do scale first, get the new center
        /// then move to new offset
        final Offset newImageCenter =
            oldScreenCropRect.center + (oldScreenDestinationRect.center - oldScreenCropRect.center) * scale;
